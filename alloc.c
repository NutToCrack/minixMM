 ...
 */
  ...
  register struct hole *hp, *prev_ptr, *max_hole, *prev_max;
  phys_clicks old_base;
  if(WorstFit == 1)
  {
    do
    {
        /* Prev max is needed to delete used block, because hp does not have to be next to max*/
        hp = hole_head;
        max_hole = hole_head;
        prev_max = NIL_HOLE;
      while (hp != NIL_HOLE && hp->h_base < swap_base)
      {
        /* Looking for biggest hole */
        ...
        if (max_hole->h_len >= clicks)
        {
          ...
          if (max_hole->h_len == 0) del_slot(prev_max, max_hole);
...
PUBLIC int do_hole_map(void)
{
  ...
  count = mm_in.m1_i1 / (sizeof(phys_clicks)*2);
  while(hp!=NIL_HOLE && i<count && hp->h_base < swap_base) {
    buffer[i] = hp->h_len;
    buffer[i+1] = hp->h_base;
    i+=2;
    hp = hp->h_next;
  }
  buffer[i] = 0;
...
{
  if (mm_in.m1_i1 == 1 || mm_in.m1_i1 == 0)
  {
